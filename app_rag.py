...
# Configuration
DATA_DIRS = ["scraped_rag_pdfs"]

st.set_page_config(page_title="Hassan RAG - PDFs", page_icon="🦍", layout="centered")

st.title("🦍 Hassan RAG PDFs")
st.caption("Votre assistant pour les documents techniques")
...
top_k_chunks = st.sidebar.slider("Précision (nb segments) :", 5, 50, 15)
...
def get_best_chunks(query, chunks, k=15):
            ...
            if api_key:
                try:
                    ...
                    completion = client.chat.completions.create(
                        model=model_name,
                        messages=[
                            {"role": "system", "content": "Tu es Hassan, un gorille assistant expert. Réponds en français en utilisant strictement le contexte fourni. Cite tes sources. Signe avec 🦍."},
                ...
                st.info("💡 Veuillez configurer votre clé API dans la barre latérale.")
        else:
            st.write("Désolé, je ne trouve pas d'informations pertinentes dans vos PDFs. 🦍")
